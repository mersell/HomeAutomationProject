...
DES_TEMP_L:     ds 1        ; Desired temperature fractional part (required by PDF)
UART_RX_TEMP:   ds 1        ; Temporary buffer for incoming UART byte
...
INIT:
	...
	; Initialize keypad FSM and debounce lock
        ...
        call    ADC_INIT
	call    UART_INIT   ; <-- YENI EKLENEN SATIR
...
MAIN_LOOP:
    ...
    call    UART_POLL       ; <-- YENI EKLENEN SATIR (Komutlari dinle ve cevapla)
    call    DISP_SHOW
    goto    MAIN_LOOP
...
DLY1:
        ...
        return
; ==========================================================
; UART MODULE (9600 Baud, 8N1, 4MHz)
; ==========================================================
UART_INIT:
        ; SPBRG calculation for 9600 baud @ 4MHz
        ; BRGH=1 (High Speed) -> Baud = Fosc / (16 * (SPBRG + 1))
        ; 9600 = 4M / (16 * 26) -> SPBRG = 25
        banksel SPBRG
        movlw   25
        movwf   SPBRG

        ; TXSTA: BRGH=1, TXEN=1, SYNC=0
        movlw   0b00100100  ; CSRC=0, TX9=0, TXEN=1, SYNC=0, BRGH=1, TRMT=1
        movwf   TXSTA

        ; RCSTA: SPEN=1, CREN=1 (Continuous Receive)
        banksel RCSTA
        movlw   0b10010000  ; SPEN=1, RX9=0, SREN=0, CREN=1
        movwf   RCSTA
        return

; ==========================================================
; UART_POLL
; Checks if data arrived. If yes, parses command per PDF Table.
; ==========================================================
UART_POLL:
        banksel PIR1
        btfss   PIR1, 5     ; Check RCIF (UART Receive Interrupt Flag)
        ...
        ; --- PARSE COMMAND (See PDF Page 16) ---
        
        ; 1. Check if it is a SET command (Bit 7 is 1)
        btfsc   UART_RX_TEMP, 7
        goto    CMD_IS_SETTER

        ; 2. Check GET commands (0x01 to 0x05)
        
        ; Case 0x01: Get desired temp low
        movf    UART_RX_TEMP, W
        xorlw   0x01
        btfsc   STATUS, 2
        goto    SEND_DES_L

        ; Case 0x02: Get desired temp high
        movf    UART_RX_TEMP, W
        xorlw   0x02
        btfsc   STATUS, 2
        goto    SEND_DES_H

        ; Case 0x03: Get ambient temp low
        movf    UART_RX_TEMP, W
        xorlw   0x03
        btfsc   STATUS, 2
        goto    SEND_AMB_L

        ; Case 0x04: Get ambient temp high
        movf    UART_RX_TEMP, W
        xorlw   0x04
        btfsc   STATUS, 2
        goto    SEND_AMB_H

        ; Case 0x05: Get fan speed
        movf    UART_RX_TEMP, W
        xorlw   0x05
        btfsc   STATUS, 2
        goto    SEND_FAN_SPD
...
SEND_FAN_SPD:
        ; PDF [R2.1.1-5]: Read fan speed. 
        ; Logic: If FAN_BIT (RD1) is ON, send 50 rps (demo), else 0.
        banksel PORTD
        btfsc   PORTD, FAN_BIT
        goto    FAN_IS_ON
        movlw   0
        ...
        return
FAN_IS_ON:
        movlw   50          ; Demo RPM value
        call    UART_TX_BYTE
        return

; --- SETTER HANDLERS ---
CMD_IS_SETTER:
        ; Mask: 10xxxxxx -> Set Low, 11xxxxxx -> Set High
        ; Check Bit 6
        btfsc   UART_RX_TEMP, 6
        goto    SET_DES_H_ACT

        ; ACTION: Set Desired Low (10t5t4t3t2t1t0)
        movf    UART_RX_TEMP, W
        andlw   0x3F        ; Extract lower 6 bits
        banksel DES_TEMP_L
        movwf   DES_TEMP_L
        return

SET_DES_H_ACT:
        ; ACTION: Set Desired High (11t5t4t3t2t1t0)
        movf    UART_RX_TEMP, W
        andlw   0x3F        ; Extract lower 6 bits
        banksel DES_TEMP_H
        movwf   DES_TEMP_H
...
; ==========================================================
; UART_TX_BYTE
; Sends W via UART (Blocking until TRMT)
; ==========================================================
UART_TX_BYTE:
        banksel TXSTA
WAIT_TX:
        btfss   TXSTA, 1    ; Check TRMT (Transmit Shift Register Status)
        goto    WAIT_TX     ; Wait until buffer is empty
        
        banksel TXREG
        movwf   TXREG       ; Send data
        
        banksel PORTA       ; Return to default bank (optional safety)
        return

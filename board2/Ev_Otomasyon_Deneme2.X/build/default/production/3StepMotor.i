...
; Note: ((PORTA) and 07Fh), 0 (LDR) and ((PORTA) and 07Fh), 1 (Pot) are configured as Analog Inputs .
...
; Full Step Sequence <((PORTB) and 07Fh), 7:4>: 1000, 0100, 0010, 0001 for CW.

step_cw:
    ; Clockwise (CW) - Sequence to turn the motor step by step.
    BANKSEL PORTB

    ; Step 1: Turn on the pin ((PORTB) and 07Fh), 7
    movlw 10000000B ; Set ((PORTB) and 07Fh), 7 to high to start the move.
    movwf PORTB ; Send the signal to the motor.
    call delay_motor ; Wait for the motor to turn.

    ; Step 2: Turn on the pin ((PORTB) and 07Fh), 6
    movlw 01000000B ; Set ((PORTB) and 07Fh), 6 to high.
    movwf PORTB ; Send the signal to the motor.
    call delay_motor ; Wait for the motor to turn.

    ; Step 3: Turn on the pin ((PORTB) and 07Fh), 5
    movlw 00100000B ; Set ((PORTB) and 07Fh), 5 to high.
    movwf PORTB ; Send the signal to the motor.
    call delay_motor ; Wait for the motor to turn.

    ; Step 4: Turn on the pin ((PORTB) and 07Fh), 4
    movlw 00010000B ; Set ((PORTB) and 07Fh), 4 to high.
    movwf PORTB ; Send the signal to the motor.
    call delay_motor ; Wait for the motor to turn.
    ...
    andlw 0x0F ; Set pins ((PORTB) and 07Fh), 4 -((PORTB) and 07Fh), 7 to 0 and keep the others as they are.
...
step_ccw:
    ...
    ; Step 1: Turn on pin ((PORTB) and 07Fh), 4
    movlw 00010000B ; Set ((PORTB) and 07Fh), 4 to high to start moving backward.
    movwf PORTB ; Send the signal to the motor.
    call delay_motor ; Wait for the motor to turn.

    ; Step 2: Turn on pin ((PORTB) and 07Fh), 5
    movlw 00100000B ; Set ((PORTB) and 07Fh), 5 to high.
    movwf PORTB ; Send the signal to the motor.
    call delay_motor ; Wait for the motor to turn.

    ; Step 3: Turn on pin ((PORTB) and 07Fh), 6
    movlw 01000000B ; Set ((PORTB) and 07Fh), 6 to high.
    movwf PORTB ; Send the signal to the motor.
    call delay_motor ; Wait for the motor to turn.

    ; Step 4: Turn on pin ((PORTB) and 07Fh), 7
    movlw 10000000B ; Set ((PORTB) and 07Fh), 7 to high.
    movwf PORTB ; Send the signal to the motor.
    call delay_motor ; Wait for the motor to turn.

...
; ----------------
uart_init:
    ; ((PORTC) and 07Fh), 7=RX input, ((PORTC) and 07Fh), 6=TX output
    BANKSEL TRISC
    bsf TRISC, 7
    bcf TRISC, 6

    ; Baud: 9600 @ 4MHz, ((TXSTA) and 07Fh), 2=1 => SPBRG=25 (0x19)
    BANKSEL SPBRG
    movlw 0x19
    movwf SPBRG

    ; TXSTA: ((TXSTA) and 07Fh), 2=1 (bit2), ((TXSTA) and 07Fh), 5=1 (bit5), async (((TXSTA) and 07Fh), 4=0)
    BANKSEL TXSTA
    movlw 0x24 ; 0010 0100b = ((TXSTA) and 07Fh), 5 + ((TXSTA) and 07Fh), 2
    movwf TXSTA

    ; RCSTA: ((RCSTA) and 07Fh), 7=1 (bit7), ((RCSTA) and 07Fh), 4=1 (bit4)
    BANKSEL RCSTA
...
; The routine waits until the transmit buffer is ready (((PIR1) and 07Fh), 4 set)
...
u_check_rx:
    ; Handle overrun error (((RCSTA) and 07Fh), 1) if happens
    BANKSEL RCSTA
    btfss RCSTA, 1 ; ((RCSTA) and 07Fh), 1 bit1
    goto u_rx_flag
    ; clear ((RCSTA) and 07Fh), 1: ((RCSTA) and 07Fh), 4=0 then ((RCSTA) and 07Fh), 4=1
    bcf RCSTA, 4
    bsf RCSTA, 4
